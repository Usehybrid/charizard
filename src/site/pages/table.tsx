...
import {fixtureUsers, type FixtureUser} from '../fixtures'
...
const columns = [
  ...
  columnHelper.accessor('software_users', {
    header: 'Users',
    cell: info => <div>{info.row.original.software_users_count}</div>,
  }),
]
...
export function SoftwareList() {
  ...
  return (
    <Table
      ...
      searchConfig={{search, setSearch, placeholder: 'Search software'}}
    />
  )
}
`
  ...
  return (
    <div>
      ...
      <p>
        ...
        header and drawer filters, sorting, pagination, row selection with bulk actions, per-row
        dropdown actions, customizable columns, sticky columns, and CSV export — each feature opts
        in through its own config prop.
      </p>

      <DemoSection
        title="Full-featured table"
        description="A kitchen-sink table wired to a module-scoped zustand store: header and drawer filters (including a date-range and tab filter), column sorting, search, pagination, checkbox row selection with bulk actions, column customization and reordering, a sticky first column, and export — all running client-side with inline data and zero network calls."
        code={tableCode}
      >
        <TableDemo />
      </DemoSection>
      ...
      <DemoSection
        ...
        description="A minimal table fed with a sanitized snapshot of the staging GET /users/team response (src/site/fixtures/users.json) — the exact row shape the console renders in production. Search filters the fixture in memory."
        code={usersTableCode}
      >
        <FixtureUsersTable />
      </DemoSection>
...
const userColumnHelper = createColumnHelper<FixtureUser>()

const userColumns = [
  userColumnHelper.accessor('first_name', {
    header: 'Name',
    cell: info => `${info.row.original.first_name} ${info.row.original.last_name}`,
  }),
  userColumnHelper.accessor('work_email', {
    header: 'Work email',
  ...
  userColumnHelper.accessor('join_date', {
    header: 'Join date',
    cell: info => {
...
const usersTableCode = `
// rows straight from the users-list API response (data key of the envelope)
const columns = [
  columnHelper.accessor('first_name', {
    header: 'Name',
    cell: info => \`\${info.row.original.first_name} \${info.row.original.last_name}\`,
  }),
  columnHelper.accessor('work_email', {header: 'Work email'}),
]
...
<Table
  ...
  columns={columns}
  loaderConfig={{isFetching: false, isError: false}}
  searchConfig={{search, setSearch, placeholder: 'Search employees'}}
/>
`

function FixtureUsersTable() {
  const [search, setSearch] = React.useState('')
  const rows = React.useMemo(() => {
    if (!search) return fixtureUsers
    const q = search.toLowerCase()
    return fixtureUsers.filter(u =>
      `${u.first_name} ${u.last_name} ${u.work_email}`.toLowerCase().includes(q),
    )
  }, [search])
